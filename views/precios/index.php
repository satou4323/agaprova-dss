<div class="row">
            ...
            <select name="mercado_id" class="form-control select2" style="width: 100%;" required>
              ...
              <?php foreach ($mercados as $m): ?>
                <option value="<?php echo $m['id']; ?>">
                  <?php echo $m['nombre']; ?> (<?php echo $m['ciudad']; ?>)
                </option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="form-group" style="max-width: 250px;">
            ...
            <div class="input-group">
              ...
              <div class="input-group-append">
                <span class="input-group-text font-weight-bold">Bs/kg</span>
              </div>
            </div>
          </div>
  ...
  <div class="col-md-6">
    <!-- WIDGET DE SUGERENCIA INTELIGENTE -->
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // 2. Interceptar Alertas y convertirlas a Toasts de SweetAlert2
    if (typeof jQuery !== 'undefined' && typeof Swal !== 'undefined') {
      ...
      if (alertSuccess.length) {
        ...
        alertSuccess.hide();
        Swal.fire({
          toast: true, position: 'top-end', showConfirmButton: false, timer: 4000, icon: 'success', title: msg
        });
      }
      if (alertError.length) {
        ...
        alertError.hide();
        Swal.fire({
          toast: true, position: 'top-end', showConfirmButton: false, timer: 5000, icon: 'error', title: msgErr
        });
      }
    }
    ...
    if (typeof jQuery !== 'undefined' && $.fn.validate) {
      $('form').validate({
        rules: {
          precio_kg: { required: true, number: true, min: 0.01 },
          mercado_id: { required: true }
        },
        messages: {
          precio_kg: { required: "Ingrese el precio", number: "Solo números", min: "Debe ser mayor a 0" },
          mercado_id: { required: "Seleccione un mercado" }
        },
        errorElement: 'span',
        errorPlacement: function (error, element) {
          error.addClass('invalid-feedback');
          element.closest('.input-group').length ? element.closest('.input-group').after(error) : element.closest('.form-group').append(error);
        },
    ...
    // 4. Renderizar Gráfico Comparativo de Barras
    <?php if (!empty($precios)): ?>
      if (typeof Chart !== 'undefined') {
        ...
        new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              y: { beginAtZero: false, suggestedMin: Math.max(0, Math.min(...dataPoints) - 5) }
            }
          }
        });
      }
